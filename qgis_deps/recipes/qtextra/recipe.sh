...
function shouldbuild_qtextra() {
    ...
    if [ "${STAGE_PATH}/qt6/plugins/sqldrivers/${LINK_libqsqlodbc}" -nt $BUILD_qtextra/.patched ]; then
        DO_BUILD=0
    fi
}
...
function build_qtextra() {
    ...
    # Check https://doc.qt.io/qt-6/sql-driver.html for available drivers
    ...
    try mkdir -p ${STAGE_PATH}/qt6/plugins/sqldrivers
    ...
    try $MAKE sub-odbc
    try cp plugins/sqldrivers/libqsqlodbc.dylib ${STAGE_PATH}/qt6/plugins/sqldrivers/

    try $MAKESMP sub-psql
    try cp plugins/sqldrivers/libqsqlpsql.dylib ${STAGE_PATH}/qt6/plugins/sqldrivers/
...
function postbuild_qtextra() {
    verify_binary qt6/plugins/sqldrivers/$LINK_libqsqlodbc
    verify_binary qt6/plugins/sqldrivers/$LINK_libqsqlpsql
}
